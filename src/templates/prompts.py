...
BORDER_VISION_REVIEW_PROMPT = """あなたはPDF帳票の罫線データ検証専門家です。
添付した **PDFページ画像（各セルの境界を赤枠でアノテーション済み）** と、以下の **テーブルセル罫線データ** を見比べて、`_borders` が実際の罫線と異なるセルのみ修正してください。

## 画像の見方

- 画像上の **赤い細線** = 自動解析が検出したテーブルセルの境界
- 各セルには `_row`/`_col` のグリッド番号が対応している
- 実際のPDF罫線（黒い線）と赤枠が一致しているかを確認する

## セルデータの構造

- `_row` / `_end_row`: セルの上端・下端グリッド行番号
- `_col` / `_end_col`: セルの左端・右端グリッド列番号
- `_borders.top/bottom/left/right`: その辺に罫線があるか（true/false）
...
1. 画像上で赤枠の辺にPDF罫線（黒線）が **ない** のに `true` になっている → `false` に修正
2. 画像上で赤枠の辺にPDF罫線（黒線）が **ある** のに `false` になっている → `true` に修正
3. セル座標（`_row`, `_end_row`, `_col`, `_end_col`）は**絶対に変更しない**
4. **修正が必要なセルのみ `corrections` に出力する**（変更なしのセルは省略する）
5. 修正不要なページは `corrections` を空配列にする
...
[
  {{
    "page_number": 1,
    "corrections": [
      {{"_row": 17, "_end_row": 18, "_col": 5, "_end_col": 7, "_borders": {{"top": true, "bottom": false, "left": true, "right": false}}}},
      {{"_row": 18, "_end_row": 19, "_col": 5, "_end_col": 7, "_borders": {{"top": false, "bottom": true, "left": true, "right": false}}}}
    ]
  }}
]

【最重要】出力は `[` で始まり `]` で終わる純粋なJSON配列文字列のみとしてください。Markdownのコードブロック(```json等)、前後の説明文、思考プロセスは一切含めないでください。"""
